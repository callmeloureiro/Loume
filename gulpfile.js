...
//////////////////////////////////////////////////////////////////////
	...
	gulp.task('build', ['copiaJS', 'copiaPHP', 'copiaCSS'])

	gulp.task('copiaJS', function() {

		var copy1 = gulp.src(['assets/js/*.min.js'])
			.pipe(gulp.dest(folder + '/assets/js/'))

    var copy2 = gulp.src('assets/img/*')
    .pipe(gulp.dest(folder + '/assets/img/'));	

    // Linguagem

		var copy3 = gulp.src('lang/*')
    .pipe(gulp.dest(folder + '/lang/'));

    return merge(copy1,copy2,copy3);

	});

	gulp.task('copiaPHP', function() {

		var copy4 = gulp.src('*.php')
    	.pipe(gulp.dest(folder + '/'));

    var copy5 = gulp.src('lib/*.php')
    	.pipe(gulp.dest(folder + '/lib/'));

    var copy6 = gulp.src('templates/*.php')
    	.pipe(gulp.dest(folder + '/templates/'));

    var copy7 =  gulp.src('templates/loureiro/*.php')
    	.pipe(gulp.dest(folder + '/templates/loureiro/'));

    return merge(copy4,copy5,copy6,copy7);

	});

	gulp.task('copiaCSS', function() {

		var copy8 = gulp.src('style.css')
	    .pipe(gulp.dest(folder + '/'));

		var copy9 = gulp.src('assets/css/*.min.css')
